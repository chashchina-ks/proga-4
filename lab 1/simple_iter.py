...
def my_genn():
    ...
    # создаем новый генератор для каждого запроса? Или можно переиспользовать?
    # Но если мы хотим независимые вызовы — лучше создавать новый каждый раз.
    # Однако, если мы хотим экономить ресурсы — можно хранить состояние между вызовами.
    # По примеру на картинке: gen.send(3), потом gen.send(5) — это один и тот же генератор, но он должен возвращать разные списки.
    # Значит, внутри coroutine мы должны каждый раз заново генерировать последовательность с начала.
